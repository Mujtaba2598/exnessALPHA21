...
async function fetchBalance(accountId) {
    try {
        ...
        // Return last known balance if any, else 0
...
// ==================== ADMIN: UPDATE TICKERALL API KEY ====================
app.post('/api/admin/set-tickerall-key', authenticate, async (req, res) => {
    try {
        ...
        // Save to config
        const newConfig = { tickerallApiKey: apiKey.trim() };
        saveConfig(newConfig);
        
        // Re-initialize TickerAll with new key
        const success = initTicker();
        
        if (success) {
            console.log('✅ TickerAll API key updated successfully');
            res.json({ success: true, message: 'TickerAll API key updated successfully! TickerAll re-initialized.' });
        } else {
            res.json({ success: false, message: 'API key saved but TickerAll initialization failed. Check the key format.' });
        }
    } catch (error) {
...
app.get('/api/debug-balance', authenticate, async (req, res) => {
    try {
        ...
        // Get raw account info from TickerAll
...
class HalalTradingEngine {
    ...
    async executeTrade(symbol, side, signal) {
        ...
        try {
            ...
            // 🔄 Fetch fresh balance every time before trading
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ Admin can update TickerAll API key without redeploying`);
